...
def assign_pe_signals(assets: List[AssetData]) -> None:
    for asset in assets:
        ...
        # --- Step 1: Calculate PE from EPS ---
        if asset.eps is not None and asset.eps != 0 and asset.price is not None:
            asset.pe_ratio = asset.price / asset.eps
        else:
            asset.pe_ratio = None

        # --- Step 2: Assign Signal ---
