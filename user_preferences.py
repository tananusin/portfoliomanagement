...
def get_user_preferences() -> UserPreference:
    ...
    st.sidebar.write(f"Core: CAGR {round(prefs.cagr_core_pct)}%, 3YGR {round(prefs.recover_core_pct)}%")
    st.sidebar.write(f"Growth: CAGR {round(prefs.cagr_growth_pct)}%, 3YGR {round(prefs.recover_growth_pct)}%")
    st.sidebar.write(f"Speculative: CAGR {round(prefs.cagr_speculative_pct)}%, 3YGR {round(prefs.recover_speculative_pct)}%")
