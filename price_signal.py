...
def assign_price_signals(assets: List[AssetData], prefs: UserPreference) -> List[AssetData]:
    ...
    for asset in assets:
        ...
        if (
            asset.price is None or
            asset.peak_1y is None or
            asset.trough_1y is None or
            asset.trough_3y is None
        ):
        ...
        if asset.drop_1y < mdd:  # dropped more than acceptable MDD
            asset.price_signal = "Overpriced"
        elif asset.gain_1y > cagr or asset.gain_3y > (1 + cagr) ** 3 - 1:
            asset.price_signal = "Underpriced"
        else:
            asset.price_signal = "-"
